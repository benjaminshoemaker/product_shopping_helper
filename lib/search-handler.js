...
export function makeDeterministicModel(scoped) {
  return (prompt) => {
    ...
    return JSON.stringify({ constraints: { query }, weights: w, items: scored })
  }
}
...
export async function rankCandidates({ model, query, weights, items, batchSize = 50, perBatchTopN = 10 }) {
  ...
  if (items.length <= batchSize) {
    ...
    return { items: data.items, meta: { numCalls } }
  }
  ...
  return { items: finalItems, meta: { numCalls } }
}
...
export async function handleSearch({ body, headers = {} }) {
  ...
  if (testMode === 'schema-error') {
    ...
    model = makeDeterministicModel({ query, weights, items: judgeItems })
  }
  ...
  const run = (async () => {
    if (judgeItems.length > batchSize) {
      const { items: ranked, meta } = await rankCandidates({ model, query, weights, items: judgeItems, batchSize, perBatchTopN })
      return { ok: true, data: { constraints: { query }, weights, items: ranked, meta } }
    } else {
  ...
  return { status: 200, body: { constraints: data.constraints || { query }, weights: data.weights, items: data.items, meta } }
}
