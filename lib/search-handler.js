...
export function makeDeterministicModel(scoped) {
  return (prompt) => {
    ...
    if (scoped.includeExplanation) {
      ...
      const [t1, t2, t3] = top
      const i1 = t1 ? findItem(t1.id) : null
      const i2 = t2 ? findItem(t2.id) : null
      const i3 = t3 ? findItem(t3.id) : null

      // Build concrete numeric contrasts when possible
      let overview = ''
      if (i1 && i2) {
        overview = `Your top result is ${i1.brand} with rating ${i1.rating.toFixed(1)} and ships in ${i1.shipDays} days. The second pick costs $${i2.price} and ships in ${i2.shipDays} days with rating ${i2.rating.toFixed(1)}.`
      } else if (i1) {
        overview = `Your top result is ${i1.brand} with rating ${i1.rating.toFixed(1)} and ${i1.shipDays}-day shipping. It best matches your filters.`
      }
      if (i3 && overview.length < 120) {
        overview += ` The third option is $${i3.price} and ships in ${i3.shipDays} days (rating ${i3.rating.toFixed(1)}).`
      }

      // Labels heuristic
      const labelFor = (it) => {
        if (!it) return 'Also good'
        if (it.shipDays === Math.min(i1?.shipDays ?? 99, i2?.shipDays ?? 99, i3?.shipDays ?? 99)) return 'Fastest arrival'
        if (it.rating === Math.max(i1?.rating ?? 0, i2?.rating ?? 0, i3?.rating ?? 0)) return 'Highest rated'
        if (it.price === Math.min(i1?.price ?? Infinity, i2?.price ?? Infinity, i3?.price ?? Infinity)) return 'Lowest price'
        return 'Also good'
      }
      const oneLine = (it) => (it ? `$${it.price} • ${it.shipDays} days • ${it.rating.toFixed(1)} rating` : '')
      const perItem = top.map((t, idx) => {
        const it = findItem(t.id)
        return { id: t.id, label: idx === 0 ? 'Best deal' : labelFor(it), oneLiner: oneLine(it) }
      })

      const caveats = []
      explanation = { overview, perItem, caveats }
    }
