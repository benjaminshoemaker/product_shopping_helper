...
export const OutputSchema = z.object({
  ...
  explanation: z
    .object({
      whyTop: z.string().optional(),
      whatCouldChange: z.array(z.string()).optional(),
...
export function buildPrompt(query, weights, items) {
  const header = [
    ...
    '- Temperature: low (0).',
    'Additionally, include an optional "explanation" block to concisely explain why the top results are ranked as they are, and what might change the ranking.',
    'Tone: concise, factual, non-salesy. 120–180 words max. Reference only provided fields (price, shipDays, features, brand, rating). No invented specs.',
    'Schema:',
    '{"constraints": {...}, "weights": {"price":0-1,"shipping":0-1,"features":0-1,"brand":0-1,"rating":0-1}, "items": [{"id": string, "overall": 0..1, "reason": string}], "explanation"?: {"whyTop"?: string, "whatCouldChange"?: string[], "missingData"?: string[]} }',
  ].join('\n')
