    ...
    .object({
      ...
      perItem: z
        .array(
          z.object({
            id: z.string(),
            label: z.enum(['Best deal', 'Also good', 'Fastest arrival', 'Highest rated', 'Lowest price']),
            oneLiner: z.string(),
          }),
        )
        .optional(),
...
export function buildPrompt(query, weights, items) {
  const header = [
    ...
    '- Temperature: low (0).',
    'Additionally, include an optional "explanation" block that offers a friendly, concrete explanation of the ranking (no speculation).',
    'Style: 8th-grade reading level. Second person. Concrete contrasts with numbers (e.g., "$98 vs $129", "ships in 2 vs 4 days", "rating 4.6"). No boilerplate.',
    'Banned phrases: ["price-to-rating balance", "based on the provided weights", "prioritized", "reflects", "overall value proposition"]. Do NOT use them.',
    'Content limits: No “what could change” section. No speculative advice. Max ~120 words across overview + oneLiners.',
    'Schema:',
    '{"constraints": {...}, "weights": {"price":0-1,"shipping":0-1,"features":0-1,"brand":0-1,"rating":0-1}, "items": [{"id": string, "overall": 0..1, "reason": string}], "explanation"?: {"overview": string, "perItem"?: [{"id": string, "label": "Best deal"|"Also good"|"Fastest arrival"|"Highest rated"|"Lowest price", "oneLiner": string}], "caveats"?: string[]} }',
  ].join('\n')
